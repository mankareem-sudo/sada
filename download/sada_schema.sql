...
-- === Seed Prompts ===
INSERT INTO "Prompt" ("id", "text", "date", "topic", "createdAt") VALUES
('cmqnseed001', 'إيه أكبر درس تعلمته في حياتك لحد دلوقتي؟', '2026-06-21', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed002', 'إيه مشروع أو حاجة جديدة بتبدأ فيها الفترة دي؟', '2026-06-22', 'career', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed003', 'إيه نصيحة كنت بتتمنى تعرفها وأنت أصغر سناً؟', '2026-06-23', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed004', 'إيه أكبر تحدي واجهته الأسبوع ده وإزاي تعاملت معاه؟', '2026-06-24', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed005', 'إيه مهارة جديدة بتحب تتعلمها وليله؟', '2026-06-25', 'learning', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed006', 'إيه أكتر كتاب أو دورة أثروا فيك؟ وليه؟', '2026-06-26', 'learning', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed007', 'إيه عادة صغيرة غيرت حياتك للأحسن؟', '2026-06-27', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed008', 'إيه أكبر خطأ ارتكبته وإيه تعلمت منه؟', '2026-06-28', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed009', 'إيه الشخصية اللي بتلهمك وإيه السبب؟', '2026-06-29', 'culture', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed010', 'إيه حاجة نجحت فيها بعد ما فشلت كتير؟', '2026-06-30', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed011', 'إيه أكتر حاجة بتخليك سعيد في يومك العادي؟', '2026-07-01', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed012', 'لو تقدر ترجع يوم واحد في حياتك، هيكون إيه وهو ليه؟', '2026-07-02', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed013', 'إيه هدف بتشتغل عليه حالياً وإيه تقدمك فيه؟', '2026-07-03', 'career', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed014', 'إيه أحلى مدحة وصلتلك في حياتك ومين اللي مدحك؟', '2026-07-04', 'life', NOW()) ON CONFLICT ("date") DO NOTHING,
('cmqnseed015', 'إيه أكتر حاجة بتخسر وقتك فيها وعايز توقفها؟', '2026-07-05', 'life', NOW()) ON CONFLICT ("date") DO NOTHING;

-- === Add unique constraint on Prompt.date (already added by Prisma, but just to be safe) ===
